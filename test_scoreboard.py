...
from ttcut_v2_3 import (
    KOKO_GAMES_BG, KOKO_NAME_MIN_FS, KOKO_POINTS_BG, KOKO_WHITE,
    build_ass, build_render, fold_full, koko_layout, koko_name_size, plan,
    scoreboard_style_for,
)
...
class ScoreboardTests(unittest.TestCase):
    ...
    def test_ass_updates_both_scores_and_game_count_from_existing_fold(self):
        ...
        self.assertTrue(any(r"\pos(613,926)" in line and line.endswith("}1")
                            for line in final))   # A wins one game
        self.assertTrue(any(r"\pos(698,926)" in line and line.endswith("}11")
                            for line in final))   # A reaches 11 points
        self.assertTrue(any(r"\pos(698,986)" in line and line.endswith("}1")
    ...
    def test_numeric_positions_stay_fixed_from_zero_to_eleven(self):
        ...
        self.assertEqual(positions, {(613, 926), (698, 926),
                                     (613, 986), (698, 986)})

    def test_long_name_only_shrinks_name_font(self):
        long_name = "許宸愷（光復國小桌球代表隊）"
        self.assertLess(koko_name_size(long_name), koko_name_size(NAMES[0]))
        self.assertGreaterEqual(koko_name_size(long_name), KOKO_NAME_MIN_FS)
        short_ass = sample()
        long_ass = sample(names=(long_name, NAMES[1]))
        short_num = [line for line in dialogue(short_ass, 2)]
        long_num = [line for line in dialogue(long_ass, 2)]
        self.assertEqual(short_num, long_num)
        self.assertEqual(dialogue(short_ass, 0), dialogue(long_ass, 0))
        self.assertIn(r"\clip(", long_ass)
    ...
    def test_1080p_and_4k_scale_all_cells_and_margins(self):
        small, large = koko_layout(1920, 1080), koko_layout(3840, 2160)
